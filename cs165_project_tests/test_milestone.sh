...
function killserver () {
    SERVER_NUM_RUNNING=`ps aux | grep $SERVER_BINARY | wc -l`
    if [ $(($SERVER_NUM_RUNNING)) -ne 0 ]; then
        # kill any servers existing
        if pgrep $SERVER_BINARY; then 
            pkill -9 $SERVER_BINARY
        fi
    fi
}
